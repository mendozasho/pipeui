...
| `pipelines.py` | `GET /pipelines/{source_id}` · `POST /pipelines/{source_id}/steps` · `DELETE /pipelines/{source_id}/steps/{step_id}` · `POST /pipelines/{source_id}/run` · `GET /pipelines/{source_id}/staging` · `GET /pipelines/{source_id}/staging/meta` (#110 preflight: `{exists, row_count, columns}`, no row materialization) · `GET /pipelines/{source_id}/export/transformed/file?format=csv\|xlsx` (#110: server-written file via DuckDB `COPY TO` / streaming openpyxl, served with `Content-Disposition: attachment` from a temp file removed by a `BackgroundTask`; 404 no staging, 422 bad format or xlsx over the 1,048,575-data-row sheet limit) · `GET /pipelines/{source_id}/export/results[?run_type]` (JSON results report) · `GET /pipelines/{source_id}/export/results/file?format=csv\|xlsx` and `GET /pipelines/sets/{set_id}/export/results/file?format=csv\|xlsx` (#152: server-written per-function validation report — one row per function × source, columns `function_name, function_type, source_name, status, rows_passed, rows_failed, pass_rate, error, result_id`; re-runs the pipeline/set so the export reflects current data; shared file tail in `middleware/results_files.py`) | E, F2 |
| `validations.py` | `POST /validations/run?function_id={id}` — fan-out: run one validation function across all sources it is attached to; returns `{ function_id, function_name, sources: [{ source_id, source_name, function_name, function_type, status, rows_passed, rows_failed, pass_rate, failing_rows, error, result_id, label }] }` (per-source entries carry the function identity — #152); 404 on unknown function · `GET /validations/{function_id}/export/results` (JSON results report) · `GET /validations/{function_id}/export/results/file?format=csv\|xlsx` (#152 file download, same report contract as the pipelines results-file routes) | F1 |
