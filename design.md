...
| ------------------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `content_hash_id`  | UUID     | the content hash lookups to this table. A UUID created from `source_name`, `primary_key`, and `ingestion_method`.                                                                                                                                                                                                                                                    | immutable            |
...
| `table_url`        | string   | the filepath to where the actual table with the data uploaded (with the columns and column types are registered) is stored. This url is used for look ups and writes into the various tables.                                                                                                                                                                        | mutable              |
...
| `content_hash_id`      | UUID   | created from `function_name`, `function_class`, and `return_type`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | immutable            |
...
| field name        | type   | description                                                                                                                        | mutable or immutable |
| ----------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `content_hash_id` | UUID   | UUID created from the `column_name`, `column_type`                                                                                 | immutable            |
...
| `content_hash_id` | UUID   | uses `param_name`, `function_id`, `paramm_type` to generate the UUID.                                                                             | immutable            |
...
- `SourceRegistryEntry` also has a method that generates the `content_hash_id` UUID based on the `source_registry_id` logic in the `source_registry` table
...
- This object should trigger or request a rollback if there was an error on the data entry.
...
2. Any functions that have the same name, signature and return type will collapse and have the same ID. This is intentional, and any function with the same ID should just overwrite in the table.
